...
# ------------------------------------------------------------------------------------#
from .Exploration import efficient_exploration, search_sampling
...
class PreExploration:
  ...
  def initialize_from_dict(self, log: logger = None, xs: CandidatePoint=None):
    ...
    log.is_verbose = options.isVerbose
    ...
    if not extend:
      ...
      search.mesh = Gmesh(pb_param=param, run_options=options) if (param.meshType).lower() == "gmesh" else Omesh(pb_param=param, run_options=options)
    ...
    if param.var_type is not None:
      ...
      for k in param.var_type:
        ...
        if k.lower()[0] == "r":
          ...
          raise IOError("Could not recognize the variable of type " + k + ". Please use on of the following keywords to identify your variable type: real, integer, discrete, categorical, ordinal, or binary")
    ...
    x_start.lambda_multipliers = param.LAMBDA
    x_start.constraints_type = param.constraints_type
    if not is_xs:
    ...
    if not extend:
      ...
      search.hashtable._is_pareto = param.isPareto
      if param.isPareto:
    ...
    if param.isPareto:
      out_p = Output(file_path=param.post_dir, vnames=param.var_names, fnames=param.fun_names, pname=param.name, runfolder=f'{param.name}_ND', suffix="Pareto")
    else:
      out_p = None
    if options.display:
    ...
    return iteration, x_start, search, options, param, post, out, B, out_p
