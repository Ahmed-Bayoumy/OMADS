...
class PrePoll:
  ...
  def initialize_from_dict(self, log: logger = None, xs: CandidatePoint=None):
    ...
    if not extend:
      ...
      poll.mesh = Gmesh(pb_param=param, run_options=options) if (param.meshType).lower() == "gmesh" else Omesh(pb_param=param, run_options=options)
    ...
    if not extend:
      ...
      poll.hashtable._is_pareto = param.isPareto
      if param.isPareto:
