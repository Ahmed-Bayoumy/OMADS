...
### input
suite_name = "bbob"
output_folder = "OMADS-optimize-fmin"
fmin = OMADS.MADS
...
for problem in suite:  # this loop will take several minutes or longer
    ...
    while (problem.evaluations < problem.dimension * budget_multiplier
        ...
        nvars = len(problem.lower_bounds)
        eval = {"blackbox": problem}
        param = {"baseline": x0,
            "lb": problem.lower_bounds,
            "ub": problem.upper_bounds,
            "var_names": [f'x{i}' for i in range(len(problem.lower_bounds))],
            "scaling": np.subtract(problem.upper_bounds, problem.lower_bounds),
            "Failure_stop": True}
        options = {"seed": 0, "budget": 1000, "tol": 1e-12, "display": False, "check_cache": True, "store_cache": True, "rich_direction": True, "psize_init": 1., "precision": "high"}

        sampling = {
                    "method": OMADS.SEARCH.explore.SAMPLING_METHOD.LH.value,
                    "ns": int((nvars+1)*(nvars+2)/2)+50,
                    "visualize": False,
                    "criterion": None
                  }
